...
const IntensityAdjuster = (props) => {
    return (
        <div className="Intensity-adjuster w-full">
            ...
            <section className="Intensity-adjuster__section h-7 grid items-end gap-x-5" style={{gridTemplateColumns: "18px 1fr 18px"}}>
                <figure className="Intensity-adjuster__figure" style={{height: "22px"}}>
                    <Icon src={LightbulbOff} alt="lightbulb off" />
                </figure>
                <IntensityMeter />
                <figure className="Intensity-adjuster__figure" style={{height: "28px"}}>
                    {props.active ? <Icon src={LightbulbOn} alt="lightbulb active" /> : <Icon src={LightbulbActive} alt="lightbulb on" />}
                </figure>
            </section>
        </div>
    );
